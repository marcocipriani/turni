...
auth.post('/login', async (c) => {
  ...
  const [u] = await db.select().from(schema.user).where(eq(schema.user.email, body.data.email.toLowerCase())).limit(1)
  // Messaggio unico: non si rivela se l'indirizzo esista.
  const generico = new HttpError(401, 'Credenziali non valide')
  if (!u || !u.attivo) throw generico
  if (!(await verifyPassword(body.data.password, u.passwordHash))) throw generico
